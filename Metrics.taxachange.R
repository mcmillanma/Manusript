...
# tributaries removed
#buid.taxachange has Attenella moved to Ephemerella for EAS Fall and Neotrichia removed from ROL Fall
...
bugs <- read_csv("bugid.taxachange.csv")
...
process_row <- function(row) {
  ...
  highest_number <- sorted_row[1]
  row_sum <- sum(sorted_row)
  p1dom <- 100*highest_number / row_sum
  return(p1dom)}

p1dom <- apply(bugs.temp, 1, process_row)
...
process_row <- function(row) {
  sorted_row <- sort(row, decreasing = TRUE)
  highest_number <- sum(sorted_row[1:2])
  row_sum <- sum(sorted_row)
  p2dom <- 100*highest_number / row_sum
  return(p2dom)}

p2dom <- apply(bugs.temp, 1, process_row)
...
process_row <- function(row) {
  sorted_row <- sort(row, decreasing = TRUE)
  highest_number <- sum(sorted_row[1:5])
  row_sum <- sum(sorted_row)
  p5dom <- 100*highest_number / row_sum
  return(p5dom)}

p5dom <- apply(bugs.temp, 1, process_row)
...
#save metrics as .csv
write.csv(metrics.temp, file="what.csv", sep = ",")
